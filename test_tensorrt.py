...
from mycode_detect_2d_3d import detect_2d_3d
import datetime
import threading
import random

categories = ['CA001','CA002','CA003','CA004','CB001','CB002','CB003','CB004','CC001','CC002','CC003','CC004','CD001','CD002','CD003','CD004']

INPUT_W = 640
INPUT_H = 640
CONF_THRESH = 0.2
IOU_THRESHOLD = 0.4
my_detect=detect_2d_3d()
...
class detect():
    ...
    def detect_one(self,img, yolov5_wrapper,depth_img,depth_intrin):
        full_scrn = False
        tic = time.clock()
        ##开始检测，并将结果写到result.jpg中
        img, result_boxes, result_scores, result_classid = yolov5_wrapper.infer(img)
        starttime=time.clock()
        result_boxes_after, result_scores_after, result_classid_after=my_detect.detect_2d_or_3d_code(depth_img,result_boxes, result_scores, result_classid,depth_intrin)
        endtime=time.clock()
        print("耗时："+str(endtime-starttime)+"(sec)")
        toc = time.clock()
        curr_fps = (toc - tic)
        #print("boxes: "+str(result_boxes_after))
        print("clss: "+str(result_classid_after))
        print("confs: "+str(result_scores_after))
        img = self.draw_boxes(img, result_boxes_after, result_scores_after, result_classid_after)
        #cv2.imwrite("result.jpg",img)
        print("time: "+str(curr_fps)+"(sec)")
        return img,result_classid_after
...
def main_one():
    ...
    engine_file_path = "/home/nano/tensorrtx/yolov5/build/yolov5l.engine"
    ...
    while True:
    	ret,frame=cap.read()
    	if ret:
    		img1=detect_one(frame,yolov5_wrapper)
    		cv2.imshow('0',frame)
    		cv2.waitKey(1)
